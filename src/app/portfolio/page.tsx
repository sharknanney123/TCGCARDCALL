...
export default async function Portfolio() {
  ...
  const [{ data: pf }, { data: positions }, { data: trades }] = await Promise.all([
    ...
    <div className="space-y-5">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div className="panel p-4"><p className="text-xs text-faded uppercase">Virtual cash</p>
          <p className="font-mono text-lg mt-1">{usd(pf?.virtual_cash)}</p></div>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.card_id}>
                ...
                <td className="td font-mono">{qty(r.quantity)}</td>
